...
def dimensionality_reduction(df_sel_data, n_comp):
    print("[INFO] feature reduction of " + str(len(df_sel_data[0])) + " for " + str(n_comp))
...
datasets_available = ["IMG_11_Labeled"]  # ["Soccer"]
...
for ds in range(0, len(dataset)):
    ...
    weights = "imagenet"
    include_top = 0
    images_path = "results/" + dataset[ds] + "/annotations"
    ...
    np.savetxt("results/deep_features/" + dataset[ds] + "/" + model_name + "/data.csv", [p for p in data],
               delimiter=',', fmt='%f')
